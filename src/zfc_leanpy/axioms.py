...
def _(): pass  # 外延性公理: 同じ要素を持つ集合は等しい

@axiom("empty_set", ALL_AXIOMS["empty_set"])
def _(): pass  # 空集合公理: 何も要素を持たない集合が存在する

@axiom("pairing", ALL_AXIOMS["pairing"])
def _(): pass  # 対公理: 任意の2集合を要素とする集合が存在する

@axiom("union", ALL_AXIOMS["union"])
def _(): pass  # 和集合公理: 集合族の和集合が存在する

@axiom("power_set", ALL_AXIOMS["power_set"])
def _(): pass  # 冪集合公理: 任意の集合の冪集合が存在する

@axiom("infinity", ALL_AXIOMS["infinity"])
def _(): pass  # 無限公理: 帰納的集合（自然数を含む）が存在する

@axiom("regularity", ALL_AXIOMS["regularity"])
def _(): pass  # 正則性公理: 空でない集合は自身と交わらない要素を持つ

@axiom("choice", ALL_AXIOMS["choice"])
def _(): pass  # 選択公理: 空でない集合族から各要素を選ぶ関数が存在する
...
def list_zfc_axioms() -> list:
    """ZFC公理名のリストを返す。"""
    return list(ALL_AXIOMS.keys())


def get_axiom(name: str):
